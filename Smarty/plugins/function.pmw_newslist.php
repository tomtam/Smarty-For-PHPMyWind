<?php
function smarty_function_pmw_newslist($params, &$smarty){
	global $dosql, $dopage;
	...
	if ($paged == 1) {
		...
		$dopage->GetPage("SELECT `title`,`picurl`, `description`, `id` FROM `#@__infolist` WHERE $where",$limit);
		...
		$dosql->Execute("SELECT `title`,`picurl`, `description`, `id` FROM `#@__infolist` WHERE $where limit 0,$limit");
		while($row = $dosql->GetArray())
		{
			if($row['picurl'])
			{
